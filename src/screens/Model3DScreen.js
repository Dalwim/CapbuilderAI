...
import { View, Button, TextInput, Image, ActivityIndicator, Alert } from 'react-native';
...
export default function Model3DScreen() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'No se pudo generar el modelo 3D. Por favor, intenta de nuevo.');
      console.error(error);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <View style={{ padding: 20 }}>
      ...
      />
      <Button 
        title={isLoading ? "Generando..." : "Generar Modelo 3D"} 
